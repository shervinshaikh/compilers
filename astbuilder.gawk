...
func add_concrete( kind, instof,   c,i,m,t ) {
	...
	for( i=1; i<=subclass_number; i++ ) 
	{
		...
		if ( subclass_type[i] == "list" ) {
			...
			Cconcrete = Cconcrete "\t    ++"m"_iter){\n";
                        Cconcrete = Cconcrete "\t\t  if(*"m"_iter != NULL) {\n"
			Cconcrete = Cconcrete "\t\t    (*"m"_iter)->m_parent_attribute = &m_attribute;\n";
                        Cconcrete = Cconcrete "\t\t  }\n"
			Cconcrete = Cconcrete "\t  }\n";
		} else {
			Cconcrete = Cconcrete "\t  "m"->m_parent_attribute = &m_attribute;\n ";
		}
	    Cconcrete = Cconcrete "\t}\n"
	}
	...
	for( i=1; i<=subclass_number; i++ ) 
	{
		...
		if ( subclass_type[i] == "list" ) {
			...
			Cconcrete = Cconcrete "\t  "m" = other."m"->clone();\n";
		}
	    Cconcrete = Cconcrete "\t}\n"
	}
	Cconcrete = Cconcrete " }\n"; 
	...
	for( i=1; i<=subclass_number; i++ ) 
	{
		...
		if ( subclass_type[i] == "list" ) {
			...
			Cconcrete = Cconcrete "\t\t    (*"m"_iter)->accept( v );\n";
                        Cconcrete = Cconcrete "\t\t  } else {\n"
                        Cconcrete = Cconcrete "\t\t    v->visitNullPointer();\n"
                        Cconcrete = Cconcrete "\t\t  }\n"
			Cconcrete = Cconcrete "\t  }\n";
		} else {
			Cconcrete = Cconcrete "\t  "m"->accept( v );\n ";
		}
	    Cconcrete = Cconcrete "\t} else {\n"
	    Cconcrete = Cconcrete "\t\tv->visitNullPointer();\n"
	    Cconcrete = Cconcrete "\t}\n"
...
func print_all_h() {
	...
	print Hvisitor >> outfile; 
        print "  virtual void visitNullPointer() {}" >> outfile;
	print "};\n" >> outfile;
